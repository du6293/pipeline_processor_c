...
Reg_out regfile( Reg_in reg_in, uint32_t *reg_data){
	...
	result.rs1_dout = (reg_in.reg_write && (reg_in.rs1 == reg_in.rd)) ?  reg_in.rd_din : ((reg_in.rs1 !=0) ? reg_in.reg_data[reg_in.rs1] : 0 ); 
	result.rs2_dout = (reg_in.reg_write && (reg_in.rs2 == reg_in.rd)) ?  reg_in.rd_din : ((reg_in.rs2 !=0) ? reg_in.reg_data[reg_in.rs2] : 0 ); 
